# Writing Style — AIOS Module Installer
...
- Celebrate wins ("Done — Claude will now self-check every piece of writing before showing it to you.")
...
**Pacing:**
- Do NOT rush. Pause after installation.
- After installation: "Files are in place. Let's run a quick test."
- After test: "You're all set. Here's what changed."

**Error handling:**
- If `.claude/` folder doesn't exist → they need Claude Code installed first
- If files already exist at the target path → ask before overwriting

---

## OVERVIEW

This module stops Claude from writing like an AI.

Every piece of writing Claude produces — emails, reports, strategy docs, marketing copy, newsletters, summaries — tends to have the same tells: em dashes everywhere, words like "leverage" and "delve", binary contrast formulas ("it's not X, it's Y"), uniform paragraph lengths, and filler transitions like "Furthermore" and "Moreover". It reads like a language model wrote it. Because one did.

This skill fixes that. It gives Claude a complete set of rules to follow and a self-check protocol to run before delivering any prose output. The result sounds like a sharp human writer, not a chatbot.

**What it does:**
- Enforces 12 core rules (no em dashes, vary sentence length, start with content, etc.)
- Bans 40+ specific words that appear 50-150x more often in AI writing than human writing
- Eliminates structural patterns that mark AI output (binary contrasts, dramatic fragmentation, rule of three)
- Adds a self-check Claude runs before every text output — scans for em dashes, banned words, filler transitions, vague claims
- Works automatically on every text output without needing to ask

**Where it installs:** Into `.claude/skills/` in your current workspace.

**Setup time:** 2 minutes. No API keys. No scripts. Just files.
...
This module is a single skill — there's nothing to configure or skip.

RECOMMENDED (and only option):
- The writing-style skill (auto-activates on all prose output)

Ask: "Ready to install? It'll take about 2 minutes."
...
Ask: "Everything looks good. Installing now."
...
### Step 2: Install the writing-style skill

Write the following file to `.claude/skills/writing-style/SKILL.md`:
...
[VERIFY]
```bash
cat .claude/skills/writing-style/SKILL.md | head -5
```
...
This module has no scripts to run. The test is asking Claude to write something and checking the output.

Ask Claude: "Write a short paragraph about why AI tools are changing how small businesses work."

**What to look for:**
- No em dashes
- No words like "leverage", "transformative", "seamless", "robust"
- No opener like "In today's fast-paced world..."
- No binary contrast pattern ("It's not X, it's Y")
- Varied sentence lengths
- Specific, concrete claims — not vague generalities

If the paragraph passes those checks, the skill is working. If Claude slips into old patterns, paste the offending text back and say "This still reads like AI — fix it." It should self-correct.

---

## WHAT'S NEXT

The skill auto-activates whenever Claude writes prose — no need to ask. It applies to:

- Strategy documents and briefs
- Marketing copy and website text
- Newsletters and emails
- Reports and summaries
- Content concepts and scripts
- Any other text output

If you ever want to push Claude harder on a specific piece, say: "Run the writing style self-check on this and fix everything that fails." It'll go through the 8-point checklist line by line.
