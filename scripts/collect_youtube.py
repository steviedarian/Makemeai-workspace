"""
DataOS — YouTube Collector (MakeMeAI Channel)

Collects daily channel stats and recent video performance for the MakeMeAI YouTube channel.

Requires:
    YOUTUBE_API_KEY      — console.cloud.google.com/apis/credentials
    YOUTUBE_CHANNEL_ID   — youtube.com/account_advanced
...
def collect():
    ...
    if not channel_id:
        return {
            "source": "youtube", "status": "skipped",
            "reason": "Missing YOUTUBE_CHANNEL_ID — add it to .env"
        }
...
if __name__ == "__main__":
    ...
    if result["status"] == "success":
        ...
        print(f"Total videos: {ch['total_videos']:,}")
        print(f"Total views: {ch['total_views']:,}")
        print(f"Videos (last 30d): {result['data']['videos_published_30d']}")
        print(f"Views (last 30d): {result['data']['total_views_30d']:,}")
    else:
