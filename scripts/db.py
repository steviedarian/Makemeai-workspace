...
def init_db():
    ...
    # Audit log -- structured security and access event logging
    conn.execute("""
    ...
    # Leads table -- inbound leads from scanner, survey, LinkedIn, referrals
    conn.execute("""
        CREATE TABLE IF NOT EXISTS leads (
            ...
            source TEXT DEFAULT 'scanner'
                CHECK(source IN ('scanner', 'survey', 'linkedin', 'referral', 'website', 'consulting_partner', 'apify', 'other')),
            ...
            bant_timeline INTEGER DEFAULT 3,
            bant_total INTEGER DEFAULT 0,
            lead_status TEXT DEFAULT 'new'
                CHECK(lead_status IN ('new', 'contacted', 'qualified', 'converted', 'dead')),
            priority_tier TEXT DEFAULT 'cool'
                CHECK(priority_tier IN ('hot', 'warm', 'cool', 'cold')),
    ...
    # Apollo usage tracking — prevent exceeding monthly limits
    conn.execute("""
